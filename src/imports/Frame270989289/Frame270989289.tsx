import imgContainer from "./58b059b979a564c3f70557c331cb54b5e258dd48.png";
import imgContainer1 from "./02203f4f61aa30d19120aa7df16dae07061dd01f.png";
import imgFrame270989294 from "./a83d6113c61a6a5429e4b85d05513e17e34b7d3a.png";
import imgContainer2 from "./5dfcbac1716ed4b7f6d200a9779b11a60077d470.png";

function Container() {
  return <div className="h-[312px] relative rounded-[28px] shrink-0 w-[320px]" data-name="Container" />;
}

function Container1() {
  return (
    <div className="h-[174px] relative rounded-[28px] shrink-0 w-[320px]" data-name="Container">
      <div aria-hidden className="absolute inset-0 pointer-events-none rounded-[28px]">
        <div className="absolute bg-[#e9f1ff] inset-0 rounded-[28px]" />
        <img alt="" className="absolute max-w-none object-cover rounded-[28px] size-full" src={imgContainer} />
        <img alt="" className="absolute max-w-none object-cover rounded-[28px] size-full" src={imgContainer1} />
      </div>
    </div>
  );
}

function Frame3() {
  return (
    <div className="content-stretch flex flex-col gap-[24px] items-start justify-center relative shrink-0">
      <Container />
      <Container1 />
    </div>
  );
}

function Frame4() {
  return (
    <div className="h-[510px] relative rounded-[28px] shrink-0 w-[320px]">
      <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-[28px]">
        <img alt="" className="absolute h-[176.48%] left-[-53.9%] max-w-none top-[-29.6%] w-[187.44%]" src={imgFrame270989294} />
      </div>
    </div>
  );
}

function Frame5() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame3 />
      <Frame4 />
    </div>
  );
}

function Frame1() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full">
      <div className="[word-break:break-word] flex flex-[1_0_0] flex-col font-['Google_Sans_Flex:Medium','Noto_Sans:Medium',sans-serif] font-medium justify-center leading-[0] min-w-px not-italic relative text-[#191c1d] text-[20px] whitespace-pre-wrap" style={{ fontVariationSettings: '"GRAD" 0, "ROND" 0, "wdth" 100' }}>
        <p className="leading-[26px] mb-0">{`Вне работы люблю исследовать этот мир, экстремальный отдых `}</p>
        <p className="leading-[26px]">и 3D-печать. Это помагает восстановить силы и набраться новыми идеями.</p>
      </div>
    </div>
  );
}

function Frame2() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start justify-center min-h-px relative w-full">
      <Frame1 />
    </div>
  );
}

function Frame6() {
  return (
    <div className="content-stretch flex flex-col gap-[10px] items-start relative self-stretch shrink-0 w-[664px]">
      <Frame5 />
      <Frame2 />
    </div>
  );
}

function Container2() {
  return (
    <div className="flex-[1_0_0] h-[664px] min-w-px relative rounded-[28px]" data-name="Container">
      <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-[28px]">
        <img alt="" className="absolute h-[211.04%] left-[-18.53%] max-w-none top-[-26.95%] w-[149.29%]" src={imgContainer2} />
      </div>
      <div className="flex flex-col justify-center size-full">
        <div className="relative size-full" />
      </div>
    </div>
  );
}

export default function Frame() {
  return (
    <div className="content-stretch flex gap-[24px] items-start justify-center relative size-full">
      <Frame6 />
      <Container2 />
    </div>
  );
}